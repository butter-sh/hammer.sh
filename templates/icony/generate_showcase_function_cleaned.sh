...
generate_showcase() {
    ...
    # Try to find myst.sh in various locations
    if [[ -f "$SCRIPT_DIR/.arty/libs/myst.sh/myst.sh" ]]; then
        myst_cmd="bash $SCRIPT_DIR/.arty/libs/myst.sh/myst.sh"
        use_myst=true
    ...
    # Verify we have the template file
    if [[ "$use_myst" == true ]] && [[ ! -f "$SCRIPT_DIR/index.html.myst" ]]; then
        log_warn "myst.sh found but index.html.myst template missing"
        use_myst=false
    fi
    ...
    if [[ "$use_myst" == true ]]; then
        ...
        # Create temporary JSON file for myst variables
        local temp_vars=$(mktemp)
        cat > "$temp_vars" << JSONEOF
{
  "font_name": "$font_name",
...
JSONEOF
        ...
        echo -e "$icons_html" > "$temp_icons_html"
        
        # Render with myst.sh
        if $myst_cmd render "$SCRIPT_DIR/index.html.myst" \
            --vars "$temp_vars" \
            --set icon_grid_html="$(cat $temp_icons_html)" \
            --output "$html_file" 2>/dev/null; then
            log_success "Showcase generated with myst.sh: $html_file"
        else
            log_warn "myst.sh rendering failed, falling back to bash generation"
            use_myst=false
        fi
        
        rm -f "$temp_vars" "$temp_icons_html"
    fi
    
    # Fallback to bash generation if myst.sh not available or failed
    if [[ "$use_myst" != true ]]; then
        log_info "Using bash template generation"
        
        # Use the bash-based HTML generation
...
HTMLEOF
        ...
        # Replace icon grid HTML
        # Use a temp file to avoid issues with special characters
        echo -e "$icons_html" > /tmp/icons_temp.html
