...
# Source the showcase generation function (supports myst.sh)
if [[ -f "$SCRIPT_DIR/generate_showcase_function.sh" ]]; then
    source "$SCRIPT_DIR/generate_showcase_function.sh"
else
    # Fallback: basic showcase generation without myst
    generate_showcase() {
        local output_dir="$1"
        local font_name="$2"
        local html_file="$output_dir/index.html"
        
        log_warn "generate_showcase_function.sh not found, using basic HTML generation"
...
HTMLEOF
        ...
        log_success "Basic showcase generated: $html_file"
    }
fi
